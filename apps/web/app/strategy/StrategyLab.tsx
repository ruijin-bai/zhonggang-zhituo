...
import {demoRedTeam,demoStrategyDraft} from "@/lib/demo-operating";
const API=process.env.NEXT_PUBLIC_API_BASE_URL??"http://127.0.0.1:8000";
export default function StrategyLab({id}:{id:string}){
 const [draft,setDraft]=useState<any>(null),[red,setRed]=useState<any>(null),[busy,setBusy]=useState(""),[fallback,setFallback]=useState(false);
 async function run(kind:"generate"|"red-team"){setBusy(kind);setFallback(false);try{const r=await fetch(`${API}/api/opportunities/${id}/strategy/${kind}`,{method:"POST"});if(!r.ok)throw new Error("service unavailable");const d=await r.json();if(kind==="generate")setDraft(d);else setRed(d)}catch{setFallback(true);if(kind==="generate")setDraft(demoStrategyDraft);else setRed(demoRedTeam)}finally{setBusy("")}}
