...
export default function StrategyLab({id}:{id:string}){
 const [draft,setDraft]=useState<any>(null),[red,setRed]=useState<any>(null),[busy,setBusy]=useState("");
 async function run(kind:"generate"|"red-team"){setBusy(kind);try{const r=await fetch(`${API}/api/opportunities/${id}/strategy/${kind}`,{method:"POST"});const d=await r.json();if(kind==="generate")setDraft(d);else setRed(d)}finally{setBusy("")}}
