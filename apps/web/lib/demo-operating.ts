...
export const DEMO_FALLBACK_ALLOWED = process.env.NEXT_PUBLIC_ALLOW_DEMO_FALLBACK !== "false";
  ...
  return {
    ...
    strategy: {
      ...
      stakeholders: [
        ...
        { name: "融资机构项目团队", organization: "融资方", role: "融资条件与项目可实施性审查", influence: "high", stance: "unknown", evidence: "具体机构要求待正式来源确认", confidence: 55 },
      ],
...
export function getDemoTrackingBoard() {
  ...
  return {
    ...
    items: [{
      opportunity,
      watch: { priority: "high", owner: "市场经营负责人", next_review_at: "2026-08-30T09:00:00+00:00" },
      actions: [
...
export function getDemoBattlecard() {
  ...
  return {
    ...
    card: {
      ...
      execution: { owner: "市场经营负责人", priority: "high", actions: board.items[0].actions, alerts: board.items[0].alerts },
    },
  };
}
...
export const demoRedTeam = {
  ...
  challenge: {
    ...
    failure_modes: ["采购评价权重未知，现有差异化优势可能并非业主真正关注项", "融资方实施约束未核实，技术经营方案可能与融资条件存在偏差", "竞争格局证据不足，无法确认一体化交付能力是否构成真实差异化"],
...
export const demoStrategyDraft = {
  mode: "offline-fallback",
  draft: { ...getDemoStrategyWorkspace().strategy, assumptions: ["采购模式仍待正式来源确认", "竞争格局尚未形成充分证据"] },
};
