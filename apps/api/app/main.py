...
def opportunity_score(opportunity_id:str,db:Session=Depends(get_db)):
 item=get_opportunity(opportunity_id,db)
 if not item: raise HTTPException(404,"Opportunity not found")
...
async def discovery_batch(request:BatchScanRequest,db:Session=Depends(get_db)): return await batch_scan(request,db)
@app.post("/api/sources/ingest",response_model=IngestResult)
async def source_ingest(request:SourceIngestRequest,db:Session=Depends(get_db)): return await ingest_source(request,db)
@app.post("/api/discovery/scan",response_model=DiscoverResult)
async def discovery_scan(request:DiscoverRequest,db:Session=Depends(get_db)):
...
def pursuit_strategy_update(opportunity_id:str,request:StrategyUpsert,db:Session=Depends(get_db)):
 try:return save_strategy(opportunity_id,request,db)
 except ValueError as exc:raise HTTPException(400,str(exc)) from exc
@app.post("/api/opportunities/{opportunity_id}/strategy/generate")
async def pursuit_strategy_generate(opportunity_id:str,db:Session=Depends(get_db)):
 item=get_opportunity(opportunity_id,db)
 if not item:raise HTTPException(404,"Opportunity not found")
...
async def pursuit_strategy_red_team(opportunity_id:str,db:Session=Depends(get_db)):
 item=get_opportunity(opportunity_id,db)
 if not item:raise HTTPException(404,"Opportunity not found")
...
async def analyze_opportunity(opportunity_id:str,db:Session=Depends(get_db)):
 item=get_opportunity(opportunity_id,db)
 if not item:raise HTTPException(404,"Opportunity not found")
