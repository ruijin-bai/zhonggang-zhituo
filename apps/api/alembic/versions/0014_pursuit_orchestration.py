...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "gate_id",
            sa.String(length=36),
            sa.ForeignKey("pursuit_decision_gates.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.String(length=36),
            sa.ForeignKey("pursuit_decision_records.id", ondelete="SET NULL"),
            nullable=True,
        ),
    ...
    indexes = {
        "pursuit_workspaces": ("organization_id", "opportunity_id", "status", "lead_membership_id", "next_review_at"),
        "pursuit_participants": ("organization_id", "workspace_id", "membership_id", "participant_role", "status"),
        "pursuit_work_items": (
            "organization_id", "workspace_id", "opportunity_id", "assignee_membership_id", "status",
            "priority", "due_at", "dependency_work_item_id", "source_action_id", "created_at", "updated_at",
        ),
        "pursuit_decision_gates": ("organization_id", "workspace_id", "opportunity_id", "gate_type", "status", "due_at", "opened_at"),
        "pursuit_gate_reviews": ("organization_id", "gate_id", "reviewer_membership_id", "status", "requested_at"),
        "pursuit_decision_records": ("organization_id", "gate_id", "opportunity_id", "decision", "decided_by_membership_id", "supersedes_decision_id", "decided_at"),
    }
    ...
    for row in legacy_actions:
        ...
        if not workspace_id:
            continue
        normalized_status = "done" if row["status"] == "done" else "open"
        bind.execute(
            sa.text(
                """
